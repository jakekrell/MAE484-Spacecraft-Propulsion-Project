...
function Nozzle = CreateNozzleConical2D_G10(rc,Lc,rt1,rt,rt2,a,L,res,plt,xc_offset)
...
%     dy0 = 1/2 * (rt1^2 - (x0-xc_end)^2))^(-1/2) * (-2*(x0-xc_end));
%
%     % solving for xc_end given dy1 = dy0 at x1_end = x0_end:
%     dy1 = dy0 -->
%     -x1/sqrt(rt1^2-x1^2) = (x0-xc_end)/sqrt(rt1^2-(x0-xc_end)^2)); -->
%     -x1_end/sqrt(rt1^2-x1_end^2) . . .
%     . . . = (x0_end-xc_end)/sqrt(rt1^2-(x0_end-xc_end)^2); ........ (eq1)

%     % solving for xc_end given y1 = y0 at x1_end = x0_end:
%     y1 = y0 -->
%     rt+rt1-sqrt(rt1^2-x1^2) = sqrt(rt1^2-(x0-xc_end)^2)+(rc-rt1); -->
%     rt+rt1-sqrt(rt1^2-x1_end^2) = sqrt(rt1^2-(x0_end-xc_end)^2)+(rc-rt1); ... (eq2)
    syms x1_end xc_end
    eq1 = -x1_end/sqrt(rt1^2-x1_end^2) == (x1_end-xc_end)/sqrt(rt1^2-(x1_end-xc_end)^2);
    eq2 = rt+rt1-sqrt(rt1^2-x1_end^2) == sqrt(rt1^2-(x1_end-xc_end)^2)+(rc-rt1);
